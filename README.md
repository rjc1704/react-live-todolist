...
각 step 브랜치는 **그 단계를 시작하기 직전의 상태**입니다. 즉, 이전 step까지의 작업은 모두 포함되어 있고, 해당 step의 작업은 아직 비어 있어 강사가 라이브로 진행합니다.

학생이 따라오지 못한 경우 다음 step 브랜치로 점프하면 라이브로 진행한 작업이 그대로 반영된 상태에서 이어갈 수 있습니다.

```bash
git checkout step1      # 1단계 시작 직전 (기초 셋업만)
git checkout step5      # 5단계 시작 직전 (1~4단계 완료)
